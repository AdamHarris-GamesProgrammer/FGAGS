...
MeshData OBJLoader::Load(const char* filename, ID3D11Device* _pDevice, ID3D11DeviceContext* deviceContext, bool invertTexCoords /*= true*/)
{
	...
	if (!binaryInFile.good())
	{
		...
		else
		{
			...
			for (int i = 0; i < numMeshVertices; i++) {
				meshData.Vertices[i].Pos = meshVertices[i];
				meshData.Vertices[i].TexC = meshTexCoords[i];
				meshData.Vertices[i].Normal = meshNormals[i];
			}
			...
			for (int i = 0; i < numMeshIndices; i++) {
				meshData.Indices[i] = indicesArray[i];
			}
